...
namespace TinderApp.DbHelper
{
    ...
    public class DatabaseLinkedinJobHelperClass
    {
        ...
        public static string DB_PATH = Path.Combine(Windows.Storage.ApplicationData.Current.LocalFolder.Path, DB_FILE_NAME_COPY);
        ...
        // Retrieve Job by status
        public List<Tb_LinkedinJob> ReadAllJobsByStatus(string status)
        {
            using (var dbConn = new SQLiteConnection(DB_PATH))
            {
                var existingJob = dbConn.Query<Tb_LinkedinJob>("select * from Tb_LinkedinJob where jobEvaluationApproval ='" + status + "'");
        ...
        // Update job status
        public void UpdateJob(LinkedinJob job, string jobEvaluationApproval)
        {
            using (var dbConn = new SQLiteConnection(DB_PATH))
            {
                var existingJob = dbConn.Query<Tb_LinkedinJob>("select * from Tb_LinkedinJob where jobId =" + job.Id).FirstOrDefault();
                if (existingJob != null)
                {
                    ...
                    existingJob.JobPosterFirstName = job.JobPoster.FirstName;
                    existingJob.LocationDescription = job.LocationDescription;
                    existingJob.JobEvaluationApproval = jobEvaluationApproval;      // set status

                    dbConn.RunInTransaction(() =>
                    {
                        //dbConn.Execute("Update tb_LinkedinJob set jobEvaluationApproval = ? where jobId=?", jobEvaluationApproval, job.Id);
                    //dbConn.Execute("Update tb_LinkedinJob set jobEvaluationApproval = 'Approved'");
                        ...
                        //var ret = dbConn.ExecuteScalar<int>("select count(*) from Tb_LinkedinJob where jobEvaluationApproval = 'TESTE'");
                }
            }
        }
